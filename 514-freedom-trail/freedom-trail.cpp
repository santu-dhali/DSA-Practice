...
public:
    ...
    int helper(int in_index, int pos,
               unordered_map<char, vector<int>>& positions, string& key,
               string& ring, vector<vector<int>>& memo) {
        ...
        for (int i : positions[key[in_index]]) {
            ...
            int next_steps =
                helper(in_index + 1, i, positions, key, ring, memo);
